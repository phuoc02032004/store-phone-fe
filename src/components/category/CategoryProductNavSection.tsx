...
const CategoryProductNavSection: React.FC<CategoryProductNavSectionProps> = ({ categoryId }) => {
  ...
  if (loading) {
    return <section className="py-4 px-4 bg-gray-100 text-black overflow-x-auto flex"><div>Loading...</div></section>;
  }
        ...
        {productsToDisplay.map((product, index) => (
          <a key={index} href={product.linkHref} className="flex flex-col items-center text-center flex-shrink-0">
            ...
            <div className="flex items-center">
              <span className="text-sm font-semibold text-black">{product.text}</span>
            </div>
          </a>
        ))}
      </div>
